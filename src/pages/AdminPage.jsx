...
function EquipmentTab() {
  ...
  const [showForm, setShowForm] = useState(false)
  const [editing, setEditing] = useState(null)
  const [form, setForm] = useState({ itemName: '', itemCategory: '', specification: '', size: '' })
  const [error, setError] = useState('')
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editing) {
        await api.put(`/Equipments/${editing.equipmentId}`, { equipmentId: editing.equipmentId, ...form })
      } else {
        await api.post('/Equipments', form)
      }
      setShowForm(false)
      setEditing(null)
      setForm({ itemName: '', itemCategory: '', specification: '', size: '' })
      load()
    } catch { setError('Failed to save') }
  }
  ...
  const handleEdit = (item) => {
    setEditing(item)
    setForm({ itemName: item.itemName, itemCategory: item.itemCategory, specification: item.specification, size: item.size })
    setShowForm(true)
    setError('')
  }
  ...
  return (
    <div>
      <div className="d-flex justify-content-between mb-3">
        <h4>Equipment</h4>
        <button className="btn btn-primary" onClick={() => { setShowForm(!showForm); setEditing(null); setForm({ itemName: '', itemCategory: '', specification: '', size: '' }); setError('') }}>
          {showForm ? 'Cancel' : '+ Add'}
        </button>
      </div>
      {error && <div className="alert alert-danger">{error}</div>}
        ...
        <div className="card p-3 mb-3">
          <form onSubmit={handleSubmit}>
            <div className="row">
              ...
              <div className="col-md-3 mb-2">
                <input className="form-control" placeholder="Category" value={form.itemCategory} onChange={e => setForm({ ...form, itemCategory: e.target.value })} />
              </div>
              <div className="col-md-3 mb-2">
      ...
      <table className="table table-striped">
        <thead><tr><th>Name</th><th>Category</th><th>Size</th><th>Actions</th></tr></thead>
        <tbody>
          {items.map(item => (
